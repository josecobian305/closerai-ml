    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[10px] p-12 text-center">
          <Mail size={36} className="text-[var(--text-subtle)] mx-auto mb-3" />
          <p className="text-[var(--text-muted)]">No emails found</p>
        </div>
      ) : (
        <div className="space-y-2">
          {emails.map((e: any, i: number) => (
            <div key={i} className={`bg-[var(--bg-card)] border rounded-[10px] p-4 ${e.isStatement ? 'border-yellow-700/60' : 'border-[var(--border)]'}`}>
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm ${e.isStatement ? 'bg-yellow-900/60 text-yellow-300' : 'bg-[var(--bg-elevated)] text-[var(--text-muted)]'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[var(--text-subtle)] text-xs mt-1">{e.date ? new Date(e.date).toLocaleString() : ''}</p>
                </div>
              </div>
            </div>
          ))}
