...
export function ReportsView() {
  ...
  if (loading) return (
    <div className="space-y-4">
      ...
      {[...Array(3)].map((_, i) => <div key={i} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 animate-pulse h-40" />)}
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleRetrain} disabled={retraining}
          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-gray-800 text-white rounded-xl text-sm flex items-center gap-2">
          {retraining ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />}
          Retrain ML
        </button>
      </div>
      {retrainResult && <div className="p-3 bg-gray-900 border border-gray-800 rounded-xl text-sm text-gray-300">{retrainResult}</div>}

      {/* Agent comparison */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {agents.map((a: any) => (
          <div key={a.agent} className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
            ...
            {a.top3 && (
              <div className="mt-3">
                <p className="text-xs text-gray-500 mb-1">Top 3 Angles</p>
          ...
          {chart.length === 0 ? (
            ...
            return (
              <div key={d.date} className="flex items-center gap-3 text-sm">
                <span className="text-gray-500 w-20 text-xs">{d.date.slice(5)}</span>
                ...
                <span className="text-gray-400 text-xs w-12 text-right">{total}</span>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {Object.entries(agents[0].weights).sort((a: any, b: any) => (b[1]?.reply_rate || 0) - (a[1]?.reply_rate || 0)).slice(0, 12).map(([name, w]: any) => (
              <div key={name} className="flex items-center justify-between bg-gray-800/40 rounded-lg px-3 py-2">
                <span className="text-gray-300 text-sm">{name.replace(/_/g, ' ')}</span>
                <div className="flex items-center gap-3 text-xs">
                  <span className="text-gray-500">{w.total_sends || 0} sends</span>
                  <span className={`font-mono ${(w.reply_rate || 0) > 0.1 ? 'text-green-400' : 'text-gray-400'}`}>
